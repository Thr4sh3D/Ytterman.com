      ...
      <div className="container mx-auto px-6">
        ...
        <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {services.map((service, index) => (
            <div 
              key={index}
              className="bg-white border border-stone-200 rounded-2xl p-8 shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-1"
            >
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                <Link 
                  to={service.link}
                  className="inline-flex items-center justify-center text-amber-700 hover:text-amber-800 font-medium"
                >
                  Läs mer <ArrowRight className="w-4 h-4 ml-1" />
                </Link>
                <button
                  onClick={() => scrollToContact(service.title)}
                  className="inline-flex items-center justify-center bg-amber-700 text-white hover:bg-amber-800 px-4 py-2 rounded-lg font-medium transition-colors"
                >
                  Begär offert
                </button>
              </div>
