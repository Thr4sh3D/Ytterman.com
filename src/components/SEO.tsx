import { Helmet } from 'react-helmet-async';

interface SEOProps {
  title?: string;
  description?: string;
  keywords?: string;
  canonicalUrl?: string;
}

export const SEO = ({ 
  title = "Kontrollansvarig och BAS-U i Västernorrland – Sundsvall, Härnösand m.fl. | Tobias Ytterman",
  description = "Kontrollansvarig (KA) och BAS U/P i Västernorrland. Tobias Ytterman hjälper dig i Sundsvall, Härnösand, Sollefteå, Timrå och Kramfors.",
  keywords = "kontrollansvarig västernorrland, KA sundsvall, BAS-U härnösand, bygglov sollefteå, kontrollplan timrå, certifierad ka kramfors, byggarbetsmiljösamordnare, PBL, Tobias Ytterman",
  canonicalUrl = "https://ytterman.com"
}: SEOProps) => {
  const structuredData = {
    "@context": "https://schema.org",
    "@type": "LocalBusiness",
    "name": "Trygg Byggprocess med Ytterman",
    "description": description,
    "url": canonicalUrl,
    "telephone": "+46761118447",
    "email": "tobias@ytterman.com",
    "founder": {
      "@type": "Person",
      "name": "Tobias Ytterman",
      "jobTitle": "Kontrollansvarig & Byggarbetsmiljösamordnare",
      "telephone": "+46761118447",
      "email": "tobias@ytterman.com"
    },
    "areaServed": [
      {
        "@type": "City",
        "name": "Sundsvall",
        "addressRegion": "Västernorrland",
        "addressCountry": "SE"
      },
      {
        "@type": "City", 
        "name": "Härnösand",
        "addressRegion": "Västernorrland",
        "addressCountry": "SE"
      },
      {
        "@type": "City",
        "name": "Sollefteå", 
        "addressRegion": "Västernorrland",
        "addressCountry": "SE"
      },
      {
        "@type": "City",
        "name": "Kramfors",
        "addressRegion": "Västernorrland",
        "addressCountry": "SE"
      },
      {
        "@type": "City",
        "name": "Timrå",
        "addressRegion": "Västernorrland",
        "addressCountry": "SE"
      }
    ],
    "serviceType": [
      "Kontrollansvarig enligt PBL",
      "Byggarbetsmiljösamordnare BAS-P",
      "Byggarbetsmiljösamordnare BAS-U",
      "Kontrollplan",
      "Arbetsmiljöplan",
      "Byggprojektledning",
      "Byggkonsultation"
    ],
    "address": {
      "@type": "PostalAddress",
      "addressLocality": "Viksjö",
      "addressRegion": "Västernorrland",
      "addressCountry": "SE"
    },
    "priceRange": "18000-60000 SEK",
    "paymentAccepted": "Faktura, Banköverföring",
    "currenciesAccepted": "SEK"
  };

  return (
    <Helmet>
      <title>{title}</title>
      <meta name="description" content={description} />
      <meta name="keywords" content={keywords} />
      <link rel="canonical" href={canonicalUrl} />
      
      {/* Open Graph */}
      <meta property="og:title" content={title} />
      <meta property="og:description" content={description} />
      <meta property="og:url" content={canonicalUrl} />
      <meta property="og:type" content="website" />
      <meta property="og:locale" content="sv_SE" />
      <meta property="og:site_name" content="Trygg Byggprocess med Ytterman" />
      
      {/* Twitter Card */}
      <meta name="twitter:card" content="summary_large_image" />
      <meta name="twitter:title" content={title} />
      <meta name="twitter:description" content={description} />
      
      {/* Structured Data */}
      <script type="application/ld+json">
        {JSON.stringify(structuredData)}
      </script>
      
      {/* Additional SEO meta tags */}
      <meta name="robots" content="index, follow" />
      <meta name="author" content="Tobias Ytterman" />
      <meta name="geo.region" content="SE-Y" />
      <meta name="geo.placename" content="Västernorrland" />
      <meta name="ICBM" content="62.3908,17.3069" />
      <meta name="language" content="Swedish" />
      <meta name="revisit-after" content="7 days" />
    </Helmet>
  );
};